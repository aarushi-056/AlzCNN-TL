...
def build_bob_model(input_shape=(128, 128, 3), num_classes=5):
    """
    Bob: 5-class CNN pretrained on dementia data.
    Architecture:
      Input →
      Conv Block 1: Conv(32) → Pool → Dropout(0.25) →
      Conv Block 2: Conv(64) → Pool → Dropout(0.25) →
      Conv Block 3: Conv(64) → Pool → Dropout(0.25) →  (note: 64 not 128, per paper)
...
def plot_confusion_matrix(y_true, y_pred, class_names, filename):
    ...
    plt.title('Average Confusion Matrix — Model 3')
...
# ── Stage 1: Train Bob on 5-Class Dementia Data ───────────────────────────────

def train_bob(X_train, y_train, X_tune, y_tune,
              n_splits=10, epochs=50, batch_size=32, num_classes=5):
    """
    Train Bob (5-class) with 10-fold cross validation on dementia data.
    Returns the best Bob model (lowest validation loss).
    Dataset split: 70% train, 15% tune, 15% test (per paper).
    """
...
def train_rob(bob_model, X_train, y_train, X_test, y_test, class_names,
    ...
    plot_combined_metrics(train_acc_accum, val_acc_accum,
                          ...
                          'training_validation_metrics_model3.png')


# ── Main ──────────────────────────────────────────────────────────────────────

if __name__ == "__main__":
    base_dir = '/home/vidur/aarushi/datasetModel3'

    # ── Stage 1 data: 5-class dementia (train + tune subfolders) ──────────────
    # Classes: Mild Dementia, Moderate Dementia, Very Mild Dementia,
    #          Control, Alzheimer's
    # Split: 70% train / 15% tune / 15% test (per paper, Table 5)
    bob_train_path = os.path.join(base_dir, 'train')   # 70%
    bob_tune_path  = os.path.join(base_dir, 'tune')    # 15%

    print("Loading Bob training data (5-class dementia)...")
    ...
    # Train: 500 AD + 4060 non-AD  |  Test: 200 AD + 200 Control (per paper, Table 6)
    ...
    # ── Train Bob ─────────────────────────────────────────────────────────────
    print("\n" + "="*60)
    print("STAGE 1: Training Bob (5-class dementia classifier)")
    print("="*60)
