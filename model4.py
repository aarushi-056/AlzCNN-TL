...
def build_model(input_shape):
    """
    CNN architecture as described in the paper (Model 4):
      Input → Conv(32) → Pool → Conv(64) → Pool →
      Flatten → Dense(128) → Dropout(0.25) → Sigmoid output
    Identical architecture to Model 2, trained on MCI instead of Dementia.
...
def plot_confusion_matrix(y_true, y_pred, class_names):
    ...
    plt.ylabel('Actual')
    plt.savefig('confusion_matrix_model4.png')
    plt.close()
    print("Confusion matrix saved as 'confusion_matrix_model4.png'.")
...
def plot_combined_metrics(train_acc, val_acc, train_loss, val_loss):
    ...
    plt.tight_layout()
    plt.savefig('training_validation_metrics_model4.png')
    plt.close()
    print("Metrics plot saved as 'training_validation_metrics_model4.png'.")
...
def cross_validate(X_train, y_train, X_test, y_test, class_names,
    ...
    """
    Train with 10-fold cross validation on MCI/control data,
    then evaluate on Alzheimer's/control test set.
    Objective: assess how many AD samples the model misclassifies as MCI,
    establishing structural similarity between MCI and Alzheimer's disease.
    """
...
if __name__ == "__main__":
    base_dir   = '/home/vidur/aarushi/datasetModel4'
    train_path = os.path.join(base_dir, 'train')   # Contains: MCI/, control/
    test_path  = os.path.join(base_dir, 'test')    # Contains: alzheimers/, control/

    print("Loading training data (MCI + Control)...")
